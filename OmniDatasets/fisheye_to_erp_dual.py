...
def create_erp_image_simple(front_camera, back_camera, front_image, back_image, output_size=(1024, 2048)):
    ...
    # 创建权重图，用于混合重叠区域
    weight_map = np.zeros((height, width, 2), dtype=np.float32)
    ...
    # 创建颜色缓冲区，用于存储每个相机的贡献
    color_buffer = np.zeros((height, width, 3, 2), dtype=np.float32)
    ...
    front_trans = front_pose[3:].reshape(3, 1)
    
    # 将全局坐标转换到相机坐标系
    # 考虑旋转和平移
    # 修正：先应用旋转，再应用平移
    front_cam_xyz = front_rot_matrix.T @ xyz - front_rot_matrix.T @ front_trans
    
    # 找出朝向前相机的点（z坐标为负）
    # 注意：在相机坐标系中，z轴指向相机前方，所以z坐标为正的点是朝向相机的
    front_valid_points = front_cam_xyz[2, :] > 0
    
    # 计算到相机的距离（用于权重计算）
    front_distance = np.linalg.norm(front_cam_xyz, axis=0)
    
    # 对于有效点，计算图像坐标
    if np.any(front_valid_points):
        # 提取有效的相机坐标
        front_cam_xyz_valid = front_cam_xyz[:, front_valid_points]
        front_distance_valid = front_distance[front_valid_points]
        ...
        if np.any(front_valid_uv):
            ...
            front_distance_final = front_distance_valid[front_valid_uv]
            
            # 从图像中获取颜色
            front_colors = front_img[front_v_valid, front_u_valid].astype(np.float32)
            
            # 计算权重（距离越近权重越大）
            front_weights = 1.0 / (front_distance_final + 1e-6)
            ...
            # 存储颜色和权重
            for i in range(len(front_erp_y)):
                y, x = front_erp_y[i], front_erp_x[i]
                color_buffer[y, x, :, 0] = front_colors[i]
                weight_map[y, x, 0] = front_weights[i]
            
            print(f"前相机处理了 {np.sum(front_valid_uv)} 个像素")
    ...
    back_trans = back_pose[3:].reshape(3, 1)
    
    # 将全局坐标转换到相机坐标系
    # 考虑旋转和平移
    # 修正：先应用旋转，再应用平移
    back_cam_xyz = back_rot_matrix.T @ xyz - back_rot_matrix.T @ back_trans
    ...
    # 计算到相机的距离（用于权重计算）
    back_distance = np.linalg.norm(back_cam_xyz, axis=0)
    
    # 对于有效点，计算图像坐标
    if np.any(back_valid_points):
        # 提取有效的相机坐标
        back_cam_xyz_valid = back_cam_xyz[:, back_valid_points]
        back_distance_valid = back_distance[back_valid_points]
        ...
        if np.any(back_valid_uv):
            ...
            back_distance_final = back_distance_valid[back_valid_uv]
            
            # 从图像中获取颜色
            back_colors = back_img[back_v_valid, back_u_valid].astype(np.float32)
            
            # 计算权重（距离越近权重越大）
            back_weights = 1.0 / (back_distance_final + 1e-6)
            ...
            # 存储颜色和权重
            for i in range(len(back_erp_y)):
                y, x = back_erp_y[i], back_erp_x[i]
                color_buffer[y, x, :, 1] = back_colors[i]
                weight_map[y, x, 1] = back_weights[i]
            
            print(f"后相机处理了 {np.sum(back_valid_uv)} 个像素")
    
    # 混合两个相机的结果
    # 对于每个像素，根据权重混合两个相机的颜色
    for y in range(height):
        for x in range(width):
            w1, w2 = weight_map[y, x]
            if w1 > 0 and w2 > 0:
                # 两个相机都有贡献，根据权重混合
                w_sum = w1 + w2
                w1 /= w_sum
                w2 /= w_sum
                erp_image[y, x] = np.round(w1 * color_buffer[y, x, :, 0] + w2 * color_buffer[y, x, :, 1]).astype(np.uint8)
            elif w1 > 0:
                # 只有前相机有贡献
                erp_image[y, x] = np.round(color_buffer[y, x, :, 0]).astype(np.uint8)
            elif w2 > 0:
                # 只有后相机有贡献
                erp_image[y, x] = np.round(color_buffer[y, x, :, 1]).astype(np.uint8)
    
    # 统计混合区域的像素数量
    overlap_count = np.sum((weight_map[:, :, 0] > 0) & (weight_map[:, :, 1] > 0))
    print(f"混合区域像素数量: {overlap_count}")
...
def main():
    ...
    # 保存结果
    output_file = os.path.join(dataset_dir, f'erp_enhanced_{frame_idx:04d}.png')
    ...
    plt.title(f'ERP Image (Enhanced) - Frame {frame_idx}')
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(os.path.join(dataset_dir, f'erp_enhanced_{frame_idx:04d}_display.png'))
    print("完成!")
